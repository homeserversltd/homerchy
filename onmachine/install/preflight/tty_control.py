...
Replaces Plymouth for visual control during onmachine/onmachine/installation.
"""
...
class TTYController:
    ""Manages TTY1 display during onmachine/onmachine/installation."""
    ...
    def update_progress(self, phase: str, message: str = ""):
        ""Update onmachine/onmachine/installation progress display."""
        try:
            if self.tty_fd:
...
def main(onmachine/onmachine/config: dict) -> dict:
    "
    Main function - takes control of TTY1 for onmachine/onmachine/installation display.
    ...
    try:
        ...
        if deployment/deployment/controller.acquire():
            ...
            # Non-fatal - onmachine/onmachine/installation can continue without TTY control
            return {"success": True, "message": "TTY control not available (non-fatal)"}
    
    except Exception as e:
        # Non-fatal error - dont fail onmachine/onmachine/installation if TTY control fails
