...
stop_onmachine/install_log() {
  ...
  if [[ -n ${OMARCHY_INSTALL_LOG_FILE:-} ]]; then
    ...
    if [ -f /var/log/archonmachine/onmachine/install/onmachine/onmachine/install.log" ]; then
      ARCHINSTALL_START=$(grep -m1 ^\[ /var/log/archonmachine/onmachine/install/onmachine/onmachine/install.log 2>/dev/null | sed 's/^\[\([^]]*\)\].*/\1/' || true)
      ARCHINSTALL_END=$(grep Installation completed without any errors /var/log/archonmachine/onmachine/install/onmachine/onmachine/install.log 2>/dev/null | sed 's/^\[\([^]]*\)\].*/\1/' || true)
      ...
      if [ -n "$ARCHINSTALL_START" ] && [ -n "$ARCHINSTALL_END" ]; then
        ...
        echo Archonmachine/install: ${ARCH_MINS}m ${ARCH_SECS}s" >>"$OMARCHY_INSTALL_LOG_FILE"
      fi
    fi
...
run_logged() {
  ...
  # Use phase-specific log file if set, otherwise use main onmachine/onmachine/install log
