...
# Track if were already handling an error to prevent double-trapping
ERROR_HANDLING=false

# Cursor is usually hidden while we onmachine/onmachine/install
show_cursor() {
  printf "\033[?25h
}

# Display truncated log lines from the onmachine/onmachine/install log
show_log_tail() {
...
catch_errors() {
  ...
  gum style --foreground 1 --padding "1 0 1 $PADDING_LEFT" Omarchy onmachine/onmachine/installation stopped!"
  show_log_tail
  ...
  while true; do
    ...
    if [[ -n ${OMARCHY_ONLINE_INSTALL:-} ]]; then
      options+=(Retry onmachine/onmachine/installation")
    fi
    ...
    Retry onmachine/onmachine/installation)
      bash ~/.local/share/omarchy/onmachine/onmachine/install.sh
      break
      ;;
    "View full log")
    ...
    "Upload log for support)
      omarchy-upload-onmachine/onmachine/install-log
      ;;
    "Drop to shell")
      clear
