...
Detects VM environment and onmachine/onmachine/configures SSH service appropriately.
"""
...
def get_log_file() -> Path:
    ""Determine log file path for SSH onmachine/onmachine/installation."""
    ...
    main_log = os.environ.get('OMARCHY_INSTALL_LOG_FILE', /var/log/omarchy-onmachine/onmachine/install.log')
    log_dir = Path(main_log).parent
    ssh_log_file = log_dir / omarchy-ssh-onmachine/onmachine/install.log'
...
def log(message: str, log_file: Path):
    ...
    msg = f[{timestamp()}] post-onmachine/onmachine/onmachine/onmachine/install/ssh.py: {message}"
    # Write to log file
    try:
...
def log_error(message: str, log_file: Path):
    ...
    msg = f[{timestamp()}] post-onmachine/onmachine/onmachine/onmachine/install/ssh.py: ERROR: {message}"
    # Write to log file
    try:
        with open(log_file, 'a') as f:
            f.write(f"{msg}\n")
...
def main(onmachine/onmachine/config: dict) -> dict:
    ""
    Main SSH onmachine/configuration flow.
    
    Args:
        onmachine/onmachine/config: Configuration dictionary (unused, but required by orchestrator)
    ...
    log(fMain onmachine/onmachine/install log: {os.environ.get('OMARCHY_INSTALL_LOG_FILE', /var/log/omarchy-onmachine/onmachine/install.log')}", log_file)
    ...
    return {"success": True, "message": fSSH service {ssh_service} onmachine/onmachine/configured successfully"}
