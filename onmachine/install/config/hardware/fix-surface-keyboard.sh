...
# Module list derived from Chris McLeods manual onmachine/install instructions
# https://chrismcleod.dev/blog/onmachine/onmachine/installing-arch-linux-with-secure-boot-on-a-microsoft-surface-laptop-studio/
product_name="$(cat /sys/class/dmi/id/product_name 2>/dev/null)"
if [[ "$product_name" =~ Surface ]]; then
  echo "Detected Surface Device

  # Modules already exist in the rootfs for the onmachine/onmachine/default kernel.
