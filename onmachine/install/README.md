...
# Using main() function
state = main(onmachine/onmachine/install_path=Path(/path/to/onmachine/onmachine/install"), phase="root)

# Using Orchestrator class directly
orchestrator = Orchestrator(onmachine/onmachine/install_path=Path(/path/to/onmachine/onmachine/install"), phase="root")
state = orchestrator.run()
```
...
```
onmachine/onmachine/onmachine/install/
...
- Structure: `{child_name}.py` in the onmachine/onmachine/install directory
- Invoked via: `ChildExecutor.execute_child()`
...
```json
{
  "phases": {
    "preflight": { "order": 1 },
    "packaging": { "order": 2 },
    onmachine/onmachine/config": { "order": 3 }
  },
  "children": ["preflight", "packaging", onmachine/onmachine/config"]
}
```
...
```json
{
  "metadata": {
    ...
    "description": Root orchestrator onmachine/onmachine/configuration"
  },
  "paths": {
    "omarchy_path": "${HOME}/.local/share/omarchy",
    onmachine/onmachine/install_path": ${OMARCHY_PATH}/onmachine/onmachine/install",
    "log_file": /var/log/omarchy-onmachine/onmachine/install.log"
  },
  "execution": {
    "mode": "sequential",
    "error_handling": {
      ...
      onmachine/onmachine/config": { "continue_on_error": true }
    }
  },
  "phases": {
    "preflight": {
      "enabled": true,
      "description": Pre-onmachine/onmachine/installation checks",
      "order": 1
    }
  },
...
- File: `/var/log/omarchy-onmachine/install.log` (or path specified in onmachine/onmachine/config)
...
def main(phase_path: Path, onmachine/onmachine/config: dict) -> StateManager:
    orchestrator = Orchestrator(onmachine/onmachine/install_path=phase_path, phase="my-phase")
...
```python
# my-module.py (in onmachine/onmachine/install directory)
