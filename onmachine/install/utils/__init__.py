...
Shared utilities for the Homerchy onmachine/onmachine/installation orchestrator system.
"""
...
class StateManager:
    ...
    def __init__(self, phase: str = "root"):
        ...
        self.onmachine/onmachine/config: Dict[str, Any] = {}
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            onmachine/onmachine/config: self.onmachine/onmachine/config,
...
class ConfigLoader:
    ""JSON onmachine/onmachine/config loading with validation.""
    
    @staticmethod
    def load(onmachine/onmachine/config_path: Union[str, Path]) -> Dict[str, Any]:
        ""Load JSON onmachine/onmachine/configuration file.""
    ...
    @staticmethod
    def validate(onmachine/onmachine/config: Dict[str, Any], required_keys: List[str]) -> bool:
        ""Validate onmachine/onmachine/config has required keys.""
        missing = [key for key in required_keys if key not in onmachine/onmachine/config]
        if missing:
...
class ChildExecutor:
    ...
    def execute_child(self, child_path: Path, child_name: str, onmachine/onmachine/config: Dict[str, Any]) -> StateManager:
        ...
        if module_py.exists():
            ...
            try:
                ...
                spec = importlib.util.spec_from_file_location(fhomerchy.onmachine/onmachine/install.{child_name}", module_py)
                ...
                if hasattr(module, 'main'):
                    ...
                    result = module.main(onmachine/onmachine/config.get(child_name, {}))
            ...
            try:
                ...
                import importlib.util
                spec = importlib.util.spec_from_file_location(fhomerchy.onmachine/onmachine/install.{child_name}", index_py)
                module = importlib.util.module_from_spec(spec)
                ...
                if hasattr(module, 'main'):
                    ...
                    result = module.main(child_path, onmachine/onmachine/config.get(child_name, {}))
                    ...
                    if isinstance(result, StateManager):
                        ...
                        if result.get("status"):
                            ...
                            self.logger.warning(f[EXECUTOR] {child_name} dict has no status/success, onmachine/onmachine/defaulting to COMPLETED")
                            child_state.set_status(Status.COMPLETED)
...
class ErrorHandler:
    ...
    @staticmethod
    def should_continue_on_error(onmachine/onmachine/config: Dict[str, Any], phase: str) -> bool:
        ""Check if execution should continue on error based on onmachine/onmachine/config.""
        error_handling = onmachine/onmachine/config.get("error_handling, {})
