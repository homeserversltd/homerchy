...
def find_limine_onmachine/onmachine/config(efi: bool) -> Path:
    ""Find Limine onmachine/onmachine/config file location."""
    if efi:
...
def extract_cmdline(onmachine/onmachine/onmachine/config_path: Path) -> str:
    ""Extract cmdline from existing Limine onmachine/onmachine/config.""
...
def create_mkinitcpio_hooks_onmachine/config() -> bool:
    ""Create mkinitcpio hooks onmachine/onmachine/configuration."""
    ...
    onmachine/onmachine/config_dir = Path("/etc/mkinitcpio.conf.d)
...
def create_limine_onmachine/default_onmachine/config(cmdline: str, efi: bool) -> bool:
    Create /etc/onmachine/onmachine/onmachine/onmachine/default/limine onmachine/onmachine/configuration.""
    onmachine/onmachine/config_content = f'''TARGET_OS_NAME="Homerchy"

ESP_PATH="/boot

KERNEL_CMDLINE[onmachine/onmachine/default]="{cmdline}
...
''
    ...
    if not efi:
        lines = onmachine/onmachine/config_content.split('\n')
        filtered_lines = [line for line in lines if not line.startswith('ENABLE_UKI=') and not line.startswith('ENABLE_LIMINE_FALLBACK=)]
        onmachine/onmachine/config_content = '\n'.join(filtered_lines)
    
    try:
        process = subprocess.Popen(
            ['sudo', 'tee, /etc/onmachine/onmachine/onmachine/onmachine/default/limine],
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True
...
def create_limine_base_onmachine/config() -> bool:
    ""Create base Limine bootloader onmachine/onmachine/configuration.""
    onmachine/onmachine/config_content = '### Read more at onmachine/onmachine/config document: https://codeberg.org/Limine/Limine/src/branch/v10.x/CONFIG.md
#timeout: 3
onmachine/onmachine/default_entry: 0
...
def setup_snapper_onmachine/configs() -> bool:
    ""Set up Snapper onmachine/onmachine/configurations for root and home.""
    # Only run if not in chroot onmachine/onmachine/install
    if os.environ.get('OMARCHY_CHROOT_INSTALL):
        return True
    ...
    try:
        ...
        # Create root onmachine/onmachine/config if needed
        if 'root not in onmachine/onmachine/configs:
            subprocess.run(['sudo', 'snapper', '-c', 'root', create-onmachine/onmachine/config', '/], check=True)
        
        # Create home onmachine/onmachine/config if needed
        ...
        # Tweak onmachine/onmachine/default Snapper onmachine/configs
        ...
        for onmachine/config_file in onmachine/config_files:
            if Path(onmachine/config_file).exists():
                # Read file
                with open(onmachine/onmachine/config_file, 'r') as f:
                    content = f.read()
                ...
                # Write back
                process = subprocess.Popen(
                    ['sudo', 'tee, onmachine/onmachine/config_file],
                    stdin=subprocess.PIPE,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE,
                    text=True
...
def reenable_mkinitcpio_hooks() -> bool:
    ...
    hooks_to_reenable = [
        /usr/share/libalpm/hooks/90-mkinitcpio-onmachine/onmachine/install.hook.disabled',
...
''
    ...
    if entry_count > 0:
        # Append entries to onmachine/onmachine/config
        try:
            with open('/boot/limine.conf', 'a') as f:
                f.write('.join(entries))
            
            # Update onmachine/onmachine/default_entry to 0
            with open('/boot/limine.conf', 'r') as f:
                content = f.read()
            
            content = re.sub(r^onmachine/onmachine/default_entry:.*', onmachine/onmachine/default_entry: 0', content, flags=re.MULTILINE)
...
def onmachine/onmachine/install_limine_to_efi() -> bool:
    """Install Limine EFI files to EFI partition and create boot entry.""
    try:
        # Find Limine EFI onmachine/onmachine/binary
        limine_efi_paths = [
        ...
        if not limine_efi:
            print(WARNING: Limine EFI onmachine/onmachine/onmachine/binary not found, skipping EFI onmachine/onmachine/installation")
        ...
        # Copy Limine EFI onmachine/onmachine/binary
        ...
        print(fWARNING: EFI onmachine/onmachine/installation failed: {e}")
        return True  # Dont fail onmachine/onmachine/installation if EFI setup fails


def cleanup_efi_boot_entries() -> bool:
    ""Remove archonmachine/install-created Limine entries from EFI."""
...
def main(onmachine/onmachine/config: dict) -> dict:
    ...
    try:
        ...
        if not create_mkinitcpio_hooks_onmachine/config():
        ...
        if not limine_onmachine/config.exists():
        ...
        # Create /etc/onmachine/onmachine/onmachine/default/limine
        if not create_limine_onmachine/default_onmachine/config(cmdline, efi):
            return {"success": False, "message: Failed to create /etc/onmachine/onmachine/onmachine/onmachine/default/limine}
        
        # Create base Limine onmachine/config
        if not create_limine_base_onmachine/config():
        ...
        if efi:
            efi_onmachine/config_locations = [
                Path('/boot/EFI/BOOT/limine.conf'),
                Path("/boot/EFI/limine/limine.conf)
            ]
            for efi_config in efi_config_locations:
                efi_onmachine/onmachine/config.parent.mkdir(parents=True, exist_ok=True)
                try:
                    import shutil
        ...
        if limine_onmachine/config != Path("/boot/limine.conf) and limine_onmachine/config.exists():
        ...
        if not setup_snapper_onmachine/configs():
            return {"success": False, "message": Failed to setup Snapper onmachine/onmachine/configs"}
        ...
        if efi:
            ...
            onmachine/onmachine/install_limine_to_efi()
        
        return {"success": True, "message": fLimine and Snapper onmachine/onmachine/configured successfully ({entry_count} boot entries created)"}
