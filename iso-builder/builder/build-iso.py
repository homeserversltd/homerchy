...
def main():
    ...
    run_command(['pacman', '--config', '/configs/pacman-online.conf', '--noconfirm', '-Sy', 'omarchy-keyring'])
    ...
    # Copy releng config
    print("Copying releng config...")
    releng_source = Path('/archiso/configs/releng')
    ...
    if not configs_source.exists():
        print(f"ERROR: Configs source not found: {configs_source}", file=sys.stderr)
        sys.exit(1)
    ...
    # Copy prebuild if exists
    prebuild_source = omarchy_source / 'prebuild' if omarchy_source.exists() else Path('/prebuild')
    if prebuild_source.exists():
        prebuild_dest = omarchy_dest / 'prebuild'
        if prebuild_dest.exists():
            shutil.rmtree(prebuild_dest)
        shutil.copytree(prebuild_source, prebuild_dest)
        print(f"Copied prebuild directory from {prebuild_source}")
    elif (omarchy_dest / 'prebuild').exists():
        print("Prebuild directory found in omarchy source")
    
    # Copy log uploader
    print("Setting up log uploader...")
    log_uploader_src = omarchy_dest / 'bin' / 'omarchy-upload-log'
    log_uploader_dest_dir = build_cache_dir / 'airootfs' / 'usr' / 'local' / 'bin'
    ...
    plymouth_source = omarchy_dest / 'default' / 'plymouth'
    plymouth_dest = build_cache_dir / 'airootfs' / 'usr' / 'share' / 'plymouth' / 'themes' / 'omarchy'
    ...
    omarchy_base = omarchy_dest / 'install' / 'omarchy-base.packages'
    if omarchy_base.exists():
        with open(omarchy_base, 'r') as f:
            all_packages.extend([line.strip() for line in f if line.strip() and not line.strip().startswith('#')])
    
    # Read omarchy-other.packages
    omarchy_other = omarchy_dest / 'install' / 'omarchy-other.packages'
    if omarchy_other.exists():
        with open(omarchy_other, 'r') as f:
            all_packages.extend([line.strip() for line in f if line.strip() and not line.strip().startswith('#')])
    
    # Read archinstall.packages
    archinstall_packages = Path('/builder/archinstall.packages')
    if archinstall_packages.exists():
        with open(archinstall_packages, 'r') as f:
            all_packages.extend([line.strip() for line in f if line.strip() and not line.strip().startswith('#')])
    ...
    pacman_cmd = [
        'pacman',
        '--config', '/configs/pacman-online.conf',
        '--noconfirm',
        '-Syw',
    ...
    if missing_packages:
        ...
        print("This will cause installation failures. Please check:", file=sys.stderr)
        print("  1. Package names are correct", file=sys.stderr)
        print("  2. Packages exist in configured repositories", file=sys.stderr)
