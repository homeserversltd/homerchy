...
def install_base_system():
    ...
    # Configure reliable mirrors before syncing
    # The ISO's default mirrorlist may contain broken mirrors, so we replace it with known-good ones
    # Also update pacman.conf to use direct Server lines to prevent archinstall from using broken mirrors
    debug_log("install_base_system: Configuring reliable pacman mirrors")
    mirrorlist_path = Path('/etc/pacman.d/mirrorlist')
    mirrorlist_path.parent.mkdir(parents=True, exist_ok=True)
    
    # Use the same reliable mirrors we configure for archinstall
    # These mirrors are known to be stable and reliable
    reliable_mirrors = """##
## Arch Linux repository mirrorlist
## Generated by Homerchy Installer
##

Server = https://mirror.omarchy.org/$repo/os/$arch
Server = https://mirror.rackspace.com/archlinux/$repo/os/$arch
Server = https://geo.mirror.pkgbuild.com/$repo/os/$arch
Server = https://archlinux.mirror.liquidweb.com/$repo/os/$arch
Server = https://mirror.leaseweb.com/archlinux/$repo/os/$arch
"""
    
    # Backup original mirrorlist if it exists
    if mirrorlist_path.exists():
        mirrorlist_path.rename(mirrorlist_path.with_suffix('.mirrorlist.bak'))
        debug_log("install_base_system: Backed up original mirrorlist")
    
    # Write new mirrorlist
    mirrorlist_path.write_text(reliable_mirrors)
    log("install_base_system: Configured reliable pacman mirrors")
    
    # Verify the mirrorlist was written correctly
    if mirrorlist_path.exists():
        mirrorlist_content = mirrorlist_path.read_text()
        debug_log(f"install_base_system: Mirrorlist content (first 200 chars): {mirrorlist_content[:200]}")
        if 'mirror.omarchy.org' in mirrorlist_content:
            log("install_base_system: ✓ Mirrorlist verified - contains reliable mirrors")
        else:
            log("install_base_system: WARNING: Mirrorlist does not contain expected mirrors!")
    
    # Also update pacman.conf to ensure it uses our mirrors
    # Replace any Include = /etc/pacman.d/mirrorlist with direct Server lines
    pacman_conf_path = Path('/etc/pacman.conf')
    if pacman_conf_path.exists():
        pacman_conf_content = pacman_conf_path.read_text()
        
        # Check if pacman.conf includes mirrorlist and has [core], [extra], [multilib] sections
        if 'Include = /etc/pacman.d/mirrorlist' in pacman_conf_content:
            debug_log("install_base_system: Updating pacman.conf to use direct Server lines")
            
            # Replace Include lines with direct Server lines for each repo
            # This prevents archinstall from using a regenerated broken mirrorlist
            ...
            pacman_conf_path.rename(pacman_conf_path.with_suffix('.pacman.conf.bak'))
            pacman_conf_path.write_text(pacman_conf_content)
            log("install_base_system: Updated pacman.conf to use direct Server lines")
        else:
            debug_log("install_base_system: pacman.conf does not use Include mirrorlist, skipping update")
    ...
    # Ensure installed system has reliable mirrors
    # archinstall might have generated a new mirrorlist with broken mirrors
    installed_mirrorlist = Path('/mnt/etc/pacman.d/mirrorlist')
    installed_mirrorlist.parent.mkdir(parents=True, exist_ok=True)
    if installed_mirrorlist.exists():
        debug_log("install_base_system: Replacing mirrorlist in installed system with reliable mirrors")
    else:
        debug_log("install_base_system: Creating mirrorlist in installed system with reliable mirrors")
    installed_mirrorlist.write_text(reliable_mirrors)
    log("install_base_system: Configured reliable mirrors in installed system")
    
    # Copy pacman config (only if it doesn't already exist from archinstall)
    if not pacman_conf_target.exists():
        ...
        # Update it to use direct Server lines if it includes mirrorlist
        pacman_conf_content = pacman_conf_target.read_text()
        if 'Include = /etc/pacman.d/mirrorlist' in pacman_conf_content:
            debug_log("install_base_system: Updating installed system pacman.conf to use direct Server lines")
            pacman_conf_content = pacman_conf_content.replace(
                '[core]\nInclude = /etc/pacman.d/mirrorlist',
                '[core]\nServer = https://mirror.omarchy.org/$repo/os/$arch\nServer = https://mirror.rackspace.com/archlinux/$repo/os/$arch\nServer = https://geo.mirror.pkgbuild.com/$repo/os/$arch'
            )
            ...
            pacman_conf_target.write_text(pacman_conf_content)
            log("install_base_system: Updated installed system pacman.conf to use direct Server lines")
