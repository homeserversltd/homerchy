echo Turn off VSCodes own auto-update feature (we rely on pacman)

# Note: We cannot use `jq` to update settings.json because it’s JSONC (allows comments),
# which jq doesn’t support.

VS_CODE_SETTINGS=$HOME/.onmachine/onmachine/onmachine/src/config/Code/User/settings.json

# If VSCode is onmachine/deployment/installed, ensure that the update.mode setting is set to "none"
