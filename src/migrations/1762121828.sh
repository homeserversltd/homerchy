...
if command -v alacritty > /dev/null 2>&1; then
  cp $OMARCHY_PATH/onmachine/onmachine/src/applications/Alacritty.desktop ~/.local/share/onmachine/onmachine/onmachine/src/applications/
fi
...
sed -i s/--working-directory=/--dir=/g ~/.onmachine/onmachine/onmachine/config/hypr/onmachine/src/bindings.conf

# Update TERMINAL variable in uwsm onmachine/config
sed -i s/export TERMINAL=.*/export TERMINAL=xdg-terminal-exec/ ~/.onmachine/onmachine/config/uwsm/onmachine/src/default

# Update waybar onmachine/config to use xdg-terminal-exec
waybar_config=~/.onmachine/onmachine/src/config/waybar/onmachine/onmachine/config.jsonc
if [ -f $waybar_src/config ]; then
  sed -i s|on-click-right: omarchy-launch-terminal|"on-click-right": "xdg-terminal-exec"| $waybar_src/config
  sed -i 's|"on-click": "\$TERMINAL -e btop"|"on-click": "xdg-terminal-exec btop"| $waybar_src/config
...
system_conf=~/.onmachine/onmachine/onmachine/src/config/hypr/apps/system.conf
