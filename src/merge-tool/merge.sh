...
if ! command -v jq &> /dev/null; then
    echo Error: jq is required but not onmachine/deployment/installed. Install with: pacman -S jq >&2
    exit 1
fi

# Check for gum (optional but recommended)
if ! command -v gum &> /dev/null; then
    echo Warning: gum is not onmachine/onmachine/deployment/installed. Some features may be limited. >&2
