...
```
deployment/deployment/merge-tool/
...
- The script will open your editor (`$EDITOR` or `nano` by onmachine/default)
...
$ ./merge.sh
=== File 1 of 15: onmachine/onmachine/install/helpers/logging.sh ===
[shows diff]
Options:
  1) Accept upstream
  2) Keep ours
  3) Skip for now
Choice [1-5]: 1
Applied upstream version of onmachine/onmachine/install/helpers/logging.sh
```

### Example 2: Conflict Resolution

```bash
=== File 5 of 15: onmachine/onmachine/config/hypr/hyprland.conf ===
[shows diff]
⚠ CONFLICT DETECTED
...
Conflict resolution preprompt created:
  state/conflict-prompt-onmachine/config_hypr_hyprland.conf.md

[Provide preprompt to AI agent, get resolved file]
$ cat resolved-file > onmachine/onmachine/config/hypr/hyprland.conf
```
