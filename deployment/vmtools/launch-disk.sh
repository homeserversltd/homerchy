...
if [ ! -f "$DISK_FILE" ]; then
    ...
    echo Please run onmachine/installation first (use -L to launch ISO for fresh onmachine/onmachine/install)."
    exit 1
fi
...
echo Launching onmachine/onmachine/installed system from disk: $DISK_FILE
