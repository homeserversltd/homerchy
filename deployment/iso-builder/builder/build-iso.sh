...
cp -r /archiso/onmachine/src/configs/releng/* $build_cache_dir/
...
cp -r /onmachine/src/configs/* $build_cache_dir/
...
cp $build_cache_dir/airootfs/root/omarchy/onmachine/onmachine/onmachine/src/bin/omarchy-upload-log $build_cache_dir/airootfs/usr/local/onmachine/onmachine/onmachine/src/bin/omarchy-upload-log

# Copy the Omarchy Plymouth theme to the ISO
mkdir -p $build_cache_dir/airootfs/usr/share/plymouth/onmachine/onmachine/onmachine/src/themes/omarchy
cp -r $build_cache_dir/airootfs/root/omarchy/onmachine/onmachine/onmachine/src/default/plymouth/* $build_cache_dir/airootfs/usr/share/plymouth/onmachine/onmachine/onmachine/src/themes/omarchy/
...
all_packages+=($(grep -v ^# $build_cache_dir/airootfs/root/omarchy/onmachine/onmachine/onmachine/deployment/deployment/install/omarchy-base.packages | grep -v ^$))
all_packages+=($(grep -v ^# $build_cache_dir/airootfs/root/omarchy/onmachine/onmachine/onmachine/deployment/deployment/install/omarchy-other.packages | grep -v ^$))
all_packages+=($(grep -v ^# /builder/archdeployment/deployment/install.packages | grep -v ^$))

# Download all the packages to the offline mirror inside the ISO
mkdir -p /tmp/offlinedb
echo Downloading packages to offline mirror...
pacman --onmachine/config /onmachine/src/configs/pacman-online.conf --noconfirm -Syw ${all_packages[@]}" --cachedir $offline_mirror_dir/ --dbpath /tmp/offlinedb
...
if [ ${#missing_packages[@]} -gt 0 ]; then
  ...
  printf '  %s\n' ${missing_packages[@]}
  echo This will cause onmachine/deployment/deployment/installation failures. Please check:
  echo   1. Package names are correct
  echo   2. Packages exist in onmachine/src/configured repositories
...
# same onmachine/src/config when booted
