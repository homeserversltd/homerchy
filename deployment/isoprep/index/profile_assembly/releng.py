...
Copy base Releng onmachine/onmachine/config from submodule and cleanup unwanted onmachine/onmachine/defaults.
""
...
def copy_releng_onmachine/config(repo_root: Path, profile_dir: Path):
    ""
    Copy base Releng onmachine/onmachine/config from submodule.
    
    Args:
        repo_root: Root of the repository
        profile_dir: ISO profile directory
    """
    print(f{Colors.BLUE}Copying base Releng onmachine/onmachine/config...{Colors.NC}")
            ...
            if item.is_dir():
                ...
                try:
                    ...
                    if 'No such file or directory' not in str(e):
                        raise
        print(f{Colors.GREEN}✓ Releng onmachine/onmachine/config copied{Colors.NC}")
    else:
...
def cleanup_reflector(profile_dir: Path):
    ""
    Cleanup unwanted Releng onmachine/onmachine/defaults (reflector).
    
    Args:
        profile_dir: ISO profile directory
    """
    print(f{Colors.BLUE}Cleaning up unwanted Releng onmachine/onmachine/defaults (reflector)...{Colors.NC}")
