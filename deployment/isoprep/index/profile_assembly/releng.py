...
Copy base Releng onmachine/onmachine/config from submodule and cleanup unwanted onmachine/src/defaults.
...
def copy_releng_src/config(repo_root: Path, profile_dir: Path):
    ...
    Copy base Releng onmachine/src/config from submodule.
    
    Args:
        repo_root: Root of the repository
        profile_dir: ISO profile directory
    "
    print(f{Colors.BLUE}Copying base Releng onmachine/src/config...{Colors.NC})
            ...
            if item.is_dir():
                ...
                try:
                    ...
                    if 'No such file or directory not in str(e):
                        raise
        print(f{Colors.GREEN}✓ Releng onmachine/src/config copied{Colors.NC})
    else:
