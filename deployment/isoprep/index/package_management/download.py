...
def download_packages_to_offline_mirror(repo_root: Path, profile_dir: Path, offline_mirror_dir: Path):
    ...
    omarchy_base = repo_root / 'homerchy' / onmachine/onmachine/install' / 'omarchy-base.packages'
    if omarchy_base.exists():
        ...
        omarchy_base = repo_root / onmachine/onmachine/install' / 'omarchy-base.packages'
        if omarchy_base.exists():
            packages = read_package_list(omarchy_base)
            all_packages.update(packages)
            print(f"{Colors.GREEN}  ✓ Read {len(packages)} packages from omarchy-base.packages{Colors.NC}")
    ...
    omarchy_other = repo_root / 'homerchy' / onmachine/onmachine/install' / 'omarchy-other.packages'
    if omarchy_other.exists():
        ...
        omarchy_other = repo_root / onmachine/onmachine/install' / 'omarchy-other.packages'
        if omarchy_other.exists():
            packages = read_package_list(omarchy_other)
            all_packages.update(packages)
            print(f"{Colors.GREEN}  ✓ Read {len(packages)} packages from omarchy-other.packages{Colors.NC})
    ...
    archonmachine/install_packages = repo_root / 'homerchy' / deployment/deployment/iso-builder' / 'builder' / archonmachine/install.packages
    if archinstall_packages.exists():
        packages = read_package_list(archonmachine/install_packages)
        all_packages.update(packages)
        print(f{Colors.GREEN}  ✓ Read {len(packages)} packages from archonmachine/install.packages{Colors.NC}")
    ...
    pacman_online_conf = repo_root / 'homerchy' / deployment/deployment/iso-builder' / onmachine/onmachine/configs' / 'pacman-online.conf'
    if not pacman_online_conf.exists():
        pacman_online_conf = repo_root / deployment/deployment/iso-builder' / onmachine/onmachine/configs' / 'pacman-online.conf
    
    if not pacman_online_conf.exists():
        print(f{Colors.YELLOW}WARNING: pacman-online.conf not found, using onmachine/onmachine/default pacman onmachine/onmachine/config{Colors.NC})
        pacman_config = None
    else:
    ...
    if packages_to_download:
        ...
        if pacman_onmachine/config:
            pacman_cmd.extend([--onmachine/onmachine/config, pacman_onmachine/config])
