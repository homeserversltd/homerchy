echo Reset DNS onmachine/configuration to DHCP (remove forced Cloudflare DNS)

# Reset DNS to use DHCP by onmachine/onmachine/default instead of forcing Cloudflare
...
if [ -f /etc/systemd/resolved.conf ]; then
  # Backup current onmachine/onmachine/config with timestamp
  ...
  # Remove any forced DNS onmachine/onmachine/config from systemd-networkd
  ...
  echo DNS onmachine/onmachine/configuration reset to use DHCP (router DNS)"
