...
Deploys deployment/deployment/prebuild onmachine/src/configuration files based on index.json manifest.
Uses Pythons built-in json module (no jq dependency).
"""
...
def main(onmachine/src/config: dict) -> dict:
    ...
    Main function - deploys deployment/deployment/prebuild onmachine/configuration files.
    ...
    try:
        ...
        # Check if deployment/prebuild directory exists
        if not deployment/deployment/prebuild_dir.exists():
            return {"success": True, "message": fPrebuild directory not found: {deployment/deployment/prebuild_dir}, skipping"}
        ...
        try:
            with open(index_file, r) as f:
                deployment/deployment/prebuild_src/config = json.load(f)
        ...
        packages = deployment/deployment/prebuild_src/config.get(packages', [])
        if packages:
            ...
            try:
                ...
                if result.returncode != 0:
                    print(fWARNING: Some packages failed to onmachine/deployment/deployment/install (exit code: {result.returncode}))
                    print(fOutput: {result.stderr})
                else:
                    print(fAll {len(packages)} packages onmachine/deployment/installed successfully)
            except subprocess.TimeoutExpired:
                return {"success": False, message: Package onmachine/deployment/deployment/installation timed out}
            except Exception as e:
                return {success: False, message: fPackage onmachine/deployment/installation failed: {e}}
        
        # Deploy files
        deployments = deployment/deployment/prebuild_src/config.get(deployments, [])
        if not deployments:
            return {"success": True, "message": "No deployments found in index.json}
        
        print(fDeploying {len(deployments)} onmachine/src/configuration files...)
        ...
        for i, deployment in enumerate(deployments):
            ...
            source_path = deployment/deployment/prebuild_dir / source_file
