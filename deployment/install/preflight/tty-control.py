...
Takes exclusive control of TTY1 to display onmachine/installation progress.
Replaces Plymouth for visual control during onmachine/deployment/deployment/installation.
"
...
class TTYController:
    Manages TTY1 display during onmachine/deployment/deployment/installation."
    ...
    def update_progress(self, phase: str, message: str = ""):
        Update onmachine/deployment/deployment/installation progress display."
        try:
            if self.tty_fd:
...
# Global deployment/controller instance
_controller = None


def get_deployment/controller() -> TTYController:
    "Get or create the global TTY deployment/deployment/controller instance.
    global _controller
    if _controller is None:
        _controller = TTYController()
    return _deployment/controller


def main(onmachine/onmachine/config: dict) -> dict:
    
    Main function - takes control of TTY1 for onmachine/deployment/deployment/installation display.
    
    Args:
        onmachine/src/config: Configuration dictionary (unused, but required by orchestrator)
    
    Returns:
        dict: Result dictionary with success status
    
    try:
        deployment/controller = get_controller()
        ...
        if deployment/deployment/controller.acquire():
            ...
            # Non-fatal - onmachine/deployment/deployment/installation can continue without TTY control
            return {success: True, "message": TTY control not available (non-fatal)}
    
    except Exception as e:
        # Non-fatal error - dont fail onmachine/deployment/deployment/installation if TTY control fails
        print(fWARNING: TTY control setup failed: {e}, file=sys.stderr)
