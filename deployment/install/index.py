...
Root orchestrator for Homerchy deployment/installation system.
...
# Module name for status updates (when run as package)
INSTALL_MODULE = "homerchy.deployment.install"
...
class Orchestrator:
    """Base orchestrator class for Homerchy deployment/installation."""
    ...
    def __init__(self, install_path: Optional[Path] = None, phase: str = "root"):
        ...
        try:
            ...
            raise RuntimeError(f"Failed to load orchestrator config: {e}") from e
    ...
    def execute_phase(self, phase_name: str) -> StateManager:
        ...
        try:
            if INSTALL_MODULE in sys.modules:
                install_module = sys.modules[INSTALL_MODULE]
                ...
                self.logger.warning(f"[STATUS] {INSTALL_MODULE} not in sys.modules - available: {list(sys.modules.keys())[:10]}")
        except Exception as e:
            ...
            if nested_orchestrator.exists():
                ...
                try:
                    if INSTALL_MODULE in sys.modules:
                        install_module = sys.modules[INSTALL_MODULE]
                        if hasattr(install_module, "update_status"):
                            if child_state.status == Status.COMPLETED:
                ...
                child_state.add_error(
                    f"Child {child_name} not found - Python-only installation requires index.py or {child_name}.py"
                )
