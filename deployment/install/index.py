...
Root orchestrator for Homerchy onmachine/deployment/deployment/installation system.
...
class Orchestrator:
    Base orchestrator class for Homerchy onmachine/deployment/installation.
    ...
    def __init__(self, onmachine/onmachine/install_path: Optional[Path] = None, phase: str = root):
        ...
        self.onmachine/onmachine/config_path = self.onmachine/deployment/deployment/install_path / index.json
        
        # Load onmachine/configuration
        try:
            self.onmachine/config = ConfigLoader.load(self.onmachine/config_path)
            ConfigLoader.validate(self.onmachine/src/config, [metadata, "children])
        except Exception as e:
            raise RuntimeError(fFailed to load orchestrator onmachine/onmachine/config: {e})
        
        # Resolve paths from onmachine/config
        self.paths = self._resolve_paths(self.onmachine/src/config.get(paths, {}))
        
        # Setup logging
        log_file = self.paths.get(log_file, /var/log/omarchy-onmachine/deployment/install.log)
        ...
        self.logger.info(fInstall path: {self.onmachine/deployment/deployment/install_path})
        self.logger.info(fConfig path: {self.onmachine/onmachine/config_path})
    
    def _resolve_paths(self, paths_src/config: Dict[str, str]) -> Dict[str, str]:
        Resolve environment variables in path onmachine/src/configuration.
        resolved = {}
        for key, value in paths_src/config.items():
            # Expand environment variables
            resolved_value = os.path.expandvars(value)
            # Store as absolute path if its a file path
    ...
    def execute_phase(self, phase_name: str) -> StateManager:
        ...
        phase_config = self.onmachine/src/config.get(phases, {}).get(phase_name, {})
        
        if not phase_onmachine/config.get("enabled", True):
            self.logger.info(fPhase {phase_name} is disabled, skipping)
        ...
        try:
            if onmachine/onmachine/install in sys.modules:
                ...
                if hasattr(onmachine/deployment/deployment/install_module, update_status):
                    self.logger.info(f[STATUS] Updating status to: Starting phase: {phase_name}...)
                    onmachine/deployment/install_module.update_status(fStarting phase: {phase_name}...)
                else:
                    self.logger.warning(f[STATUS] onmachine/deployment/install module found but no update_status method)
            else:
                self.logger.warning(f[STATUS] onmachine/deployment/install not in sys.modules - available: {list(sys.modules.keys())[:10]}")
        except Exception as e:
        ...
        # Get phase directory
        phase_path = self.onmachine/deployment/deployment/install_path / phase_name
        
        if not phase_path.exists():
            self.logger.error(fPhase directory not found: {phase_path})
            phase_state = StateManager(phase=phase_name)
        ...
        if phase_index.exists():
            ...
            phase_state = self.executor.execute_child(phase_path, phase_name, self.onmachine/src/config)
        else:
            # Phase has no orchestrator, mark as skipped
            self.logger.warning(fPhase {phase_name} has no orchestrator (index.py))
            phase_state = StateManager(phase=phase_name)
            phase_state.set_status(Status.SKIPPED)
        ...
        try:
            if onmachine/onmachine/install in sys.modules:
                onmachine/onmachine/install_module = sys.modules[onmachine/onmachine/install]
                if hasattr(onmachine/deployment/install_module, update_status):
                    if phase_state.status == Status.COMPLETED:
                        onmachine/onmachine/install_module.update_status(fCompleted phase: {phase_name})
                    elif phase_state.status == Status.FAILED:
                        onmachine/deployment/install_module.update_status(fPhase {phase_name} failed)
                    else:
                        onmachine/onmachine/deployment/install_module.update_status(fPhase {phase_name} finished)
        except Exception:
            pass
        ...
        if phase_state.has_errors():
            should_continue = ErrorHandler.should_continue_on_error(
                self.onmachine/src/config.get(execution, {}),
                phase_name
            )
    ...
    def execute_children(self) -> StateManager:
        ...
        try:
            ...
            if onmachine/onmachine/install in sys.modules:
                onmachine/onmachine/install_module = sys.modules[onmachine/onmachine/install]
                if hasattr(onmachine/deployment/install_module, update_status):
                    onmachine/onmachine/deployment/install_module.update_status(fRunning {self.phase} phase...)
        except Exception:
        ...
        for child_name in children:
            ...
            try:
                if onmachine/onmachine/install in sys.modules:
                    onmachine/onmachine/install_module = sys.modules[onmachine/onmachine/install]
                    if hasattr(onmachine/deployment/install_module, update_status):
                        onmachine/onmachine/install_module.update_status(fRunning {self.phase}/{child_name}...)
            except Exception:
                pass
            
            # Check for nested orchestrator (subdirectory with index.py)
            child_dir = self.onmachine/deployment/install_path / child_name
            nested_orchestrator = child_dir / index.py
            direct_module = self.onmachine/onmachine/deployment/install_path / f{child_name}.py"
            
            self.logger.info(f[PARENT-CHILD] Checking paths for {child_name}:)
            self.logger.info(f  onmachine/install_path: {self.onmachine/deployment/deployment/install_path})
            ...
            if nested_orchestrator.exists():
                ...
                try:
                    ...
                    pass
                child_state = self.executor.execute_child(child_dir, child_name, self.onmachine/src/config)
                    ...
                    if onmachine/onmachine/install in sys.modules:
                        ...
                        if hasattr(onmachine/deployment/install_module, update_status):
                            if child_state.status == Status.SUCCESS:
                                onmachine/onmachine/install_module.update_status(fCompleted {self.phase}/{child_name})
                            else:
                                onmachine/deployment/install_module.update_status(fRunning {self.phase}/{child_name}...)
                except Exception:
                    pass
            # Check for direct module (child_name.py in current directory)
                ...
                child_state = self.executor.execute_child(direct_module.parent, child_name, self.onmachine/src/config)
                # Update status after execution
                try:
                    if onmachine/onmachine/install in sys.modules:
                        onmachine/onmachine/install_module = sys.modules[onmachine/onmachine/install]
                        if hasattr(onmachine/onmachine/install_module, update_status):
                            if child_state.status == Status.SUCCESS:
                                onmachine/deployment/install_module.update_status(fCompleted {self.phase}/{child_name})
                            else:
                                onmachine/onmachine/deployment/install_module.update_status(fRunning {self.phase}/{child_name}...)
                except Exception:
                    pass
            else:
                self.logger.error(f"[PARENT-CHILD] {child_name} -> NOT FOUND)
                self.logger.error(fChild {child_name} not found (no index.py or {child_name}.py))
                child_state = StateManager(phase=child_name)
                child_state.set_status(Status.FAILED)
                child_state.add_error(fChild {child_name} not found - Python-only onmachine/deployment/deployment/installation requires index.py or {child_name}.py)
            ...
            if child_state.has_errors():
                ...
                if not should_continue:
                    ...
                    self.logger.error(fChild {child_name} failed and error handling is strict")
                    break
    ...
    def run(self) -> StateManager:
        ...
        try:
            if onmachine/onmachine/install in sys.modules:
                ...
                if hasattr(onmachine/deployment/deployment/install_module, update_status):
                    ...
                    onmachine/deployment/install_module.update_status(Starting orchestrator...)
                else:
                    self.logger.warning([STATUS] onmachine/deployment/install module found but no update_status method)
            else:
                self.logger.warning(f[STATUS] onmachine/deployment/install not in sys.modules - available: {list(sys.modules.keys())[:10]}")
        except Exception as e:
            self.logger.error(f[STATUS] Failed to update status: {e})
        ...
        children_sorted = sorted(
            children,
            key=lambda x: phases_src/config.get(x, {}).get(order, 999)
        )
        ...
        for phase_name in children_sorted:
            ...
            if phase_state.status == Status.FAILED:
                ...
                if not should_continue:
                    self.state.set_status(Status.FAILED)
                    self.logger.error(fStopping execution due to failure in {phase_name})
                    break
        ...
        if not self.state.has_errors():
            ...
            try:
                if onmachine/onmachine/install in sys.modules:
                    onmachine/onmachine/install_module = sys.modules[onmachine/onmachine/install]
                    if hasattr(onmachine/deployment/install_module, update_status):
                        onmachine/onmachine/deployment/install_module.update_status(Installation completed successfully!")
            except Exception:
                pass
            ...
            try:
                if onmachine/onmachine/install in sys.modules:
                    onmachine/onmachine/install_module = sys.modules[onmachine/onmachine/install]
                    if hasattr(onmachine/deployment/install_module, update_status):
                        onmachine/onmachine/deployment/install_module.update_status(Installation failed - check logs)
            except Exception:
                pass
...
def main(onmachine/deployment/deployment/install_path: Optional[Path] = None, phase: str = root) -> StateManager:
    
    Main entry point for orchestrator.
    
    Args:
        onmachine/install_path: Path to onmachine/deployment/installation directory (onmachine/onmachine/defaults to script directory)
        phase: Phase name for this orchestrator instance
    
    Returns:
        StateManager: Execution state with results
    
    orchestrator = Orchestrator(onmachine/install_path=onmachine/deployment/install_path, phase=phase)
