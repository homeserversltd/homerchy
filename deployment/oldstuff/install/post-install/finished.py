...
def stop_deployment/deployment/install_log():
    ...
    try:
        ...
        # This is handled by the shell environment, but we ensure its stopped
        ...
        with open(log_file, 'a') as f:
            ...
            archinstall_log = Path(/var/log/archonmachine/deployment/install/onmachine/onmachine/install.log)
            if archinstall_log.exists():
                try:
                    with open(archdeployment/deployment/install_log, r) as af:
                        content = af.read()
                        ...
                        if start_match and end_match:
                            ...
                            if end_timestamp_match:
                                ...
                                try:
                                    ...
                                    f.write(fArchdeployment/deployment/install: {arch_mins}m {arch_secs}s\n)
                                except ValueError:
                                    pass
                except Exception:
                    pass
            ...
            if start_time_str:
                try:
                    ...
                    f.write(fOmarchy:     {omarchy_mins}m {omarchy_secs}s\n)
                    
                    # Try to calculate total if we have archinstall duration
                    # (This would require storing archdeployment/install duration, simplified here)
                except ValueError:
                    pass
            
            f.write(=================================\n)
            # NOTE: Future enhancement - we want to restart at the end of homerchy onmachine/onmachine/deployment/installation
        ...
        # Dont fail onmachine/deployment/installation if log writing fails
        print(fWarning: Failed to write log summary: {e}", file=sys.stderr)
...
def dump_logs_to_root():
    ...
    try:
        result = subprocess.run(
            ['journalctl', -u, omarchy-first-boot-onmachine/deployment/install.service, --no-pager],
    ...
    if os.path.exists(onmachine/install_log):
        try:
            with open(onmachine/deployment/install_log, r) as f:
                onmachine/onmachine/install_log_content = f.read()
            subprocess.run([cp, onmachine/deployment/install_log, /root/a.txt], check=False)
            print(  Created: /root/a.txt (full onmachine/deployment/deployment/installation log), file=sys.stderr)
        except Exception:
            pass
    ...
    log_patterns = [
        ...
        (k.txt, marker|Marker|MARKER|omarchy-onmachine/deployment/deployment/install-needed),
        (l.txt', 'orchestrator|Orchestrator|ORCHESTRATOR'),
        ('m.txt', module|Module|MODULE),
    ]
    
    # Comsrc/bine all log sources for grepping
    all_logs = 
    if journal_output:
        all_logs += journal_output + \n
    if onmachine/install_log_content:
        all_logs += onmachine/deployment/deployment/install_log_content + \n
...
def main(onmachine/onmachine/config: dict) -> dict:
    ...
    NOTE: Future enhancement - we want to restart at the end of homerchy onmachine/deployment/installation.
    ...
    try:
        # Stop onmachine/install log
        stop_onmachine/install_log()
        
        # Dump useful logs to /root for debugging
        dump_logs_to_root()
        
        # Remove sudoers file if it exists
        sudoers_file = Path(/etc/sudoers.d/99-omarchy-onmachine/deployment/deployment/installer)
